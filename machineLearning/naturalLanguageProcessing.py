...
# --- Sentence Generation Using Grammars ---
def newGrammarSentence():
    grammar = {
        "_S"  : ["_NP _VP"],
            "_NP" : ["_N",
                     "_A _NP _P _A _N"],
    ...
    def expand(grammer, tokens):
        for i, token in enumerate(tokens):
            ...
            #if we get here, we found a non terminal token, so we need to choose a random replacement
        ...
        #if we get here, we had all terminals ad are done
        return tokens
...
def p_topic_given_document(topic, d, alpha=0.1):
    """the fraction of words in document d that are assgined to topic (plus smoothing)"""
    return ((document_topic_counts[d][topic] + alpha)/(document_lengths[d] + K * alpha))

def p_word_given_topic(word, topic, beta=0.1):
    """the fratcion of words assigned to topic that equal word (plus smoothing)"""
    return ((topic_word_counts[topic][word] + beta) / (topic_counts[topic] + W * beta))

def topic_weight(d, word, k):
    """given a document ad a word in that document, return the weight for the kth topic"""
