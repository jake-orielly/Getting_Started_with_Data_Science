...
#each entry is a persons longitude, latitude and favorite language
...
def majority_vote(labels):
    """assumes that labels are ordered fomr nearest to fatherst"""
    ...
    if num_winners == 1:
        return winner   #unique winner so we return it
    else:
...
# key is language, value is pair (long, lat)
"""plots = {"Java": ([], []), "Python" : ([], []), "R" : ([], [])}
...
# --- what if we try to predict each city's preferred language using it's neighbors? ---

"""for k in [1, 3, 5, 7]: # try several values for k
    num_correct = 0
...
# --- Demo'ing the curse of dimensionality

def random_point(dim):
    return [random.random() for _ in range(dim)]

def random_distances(dim, num_pairs):
    return [fnc.distance(random_point(dim), random_point(dim))
