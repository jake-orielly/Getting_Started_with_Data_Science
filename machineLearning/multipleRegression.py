...
def estimate_beta(x, y):
    ...
    return fnc.minimize_stochastic(squared_error,
...
def multiple_r_squared(x, y, beta):
    ...
    return 1.0 - sum_of_squared_errors / fnc.total_sum_of_squares(y)
...
#101 points half near 0 half near 200
...
bootstrap_betas = bootstrap_statistic(zip(x, daily_minutes_good),
                                      ...
                                      100)
boostrap_standard_errors = [
    fnc.standard_deviation([beta[i] for beta in bootstrap_betas])
    for i in range(4)]

print(boostrap_standard_errors)"""

def p_value(beta_hat_j, sigma_hat_j):
    if beta_hat_j > 0:
        #if the coefficient is positive, we need to compute twice
        #the probability of seeing an even larger value
        return 2 * (1 - fnc.normal_cdf(beta_hat_j / sigma_hat_j))
    else:
        #otherwise twice the odds of smaller value
        return 2 * fnc.normal_cdf(beta_hat_j / sigma_hat_j)

#print(p_value(0.911, 0.990))

#alpha is a *hperparameter* controlling how harsh the pneality is
